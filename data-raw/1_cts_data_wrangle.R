...
preSegment = function(observations, tracks, transects) {
  ...
  shp.tidy.data = shp.tidy %>% mutate(id = as.integer(id),
    ...
    select(-group) %>% group_by(id) %>% mutate(piece = as.integer(factor(zoo::na.locf(piece)))) %>% ungroup %>%
  ...
  shp.tidy.data.new = shp.tidy.data %>% mutate(new_transect_id = ifelse(group %in% id.transects$group, NA, transect_i),
    ...
    select(-group) %>% group_by(dataset_id) %>% mutate(new_transect_id = zoo::na.locf(new_transect_id)) %>%
    group_by(new_transect_id) %>% mutate(piece = as.integer(factor(zoo::na.locf(piece))),
  ...
  nwasc.ph1.obs.pre <<- do.call(rbind, lapply(obs.dat.new$st_astext, rgeos::readWKT)) %>% as.data.frame %>%
