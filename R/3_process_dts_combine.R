...
phase1_segmentDTS = function(observations, transects, v.spd = 10, occurences = FALSE) {
  ...
  dts.time = transects %>%
    ...
    mutate(start_tm = as.POSIXct(lubridate::ymd(start_dt, quiet = TRUE) + lubridate::hms(start_tm, quiet = TRUE)),
  ...
  obs.time = observations %>%
    ...
    mutate(obs_start_tm = as.POSIXct(lubridate::ymd(obs_dt, quiet = TRUE) + lubridate::hms(obs_start_tm, quiet = TRUE)),
  ...
  midpoints.point = transects %>% filter(grepl("POINT", st_astext), !is.na(heading_tx))
    if (nrow(midpoints.point) != 0) {
      midpoints.point = midpoints.point %>% bind_cols(., as.data.frame(do.call(rbind, lapply(.$st_astext, rgeos::readWKT)))) %>%
        select(-st_astext) %>% rename(long = x, lat = y) %>%
        mutate(half_dist_m = transect_time_min_nb * traversal_speed_nb * 1000 / 120) %>%
        rowwise %>% mutate(coords_mid = list(geosphere::destPoint(c(long, lat), heading_tx, half_dist_m))) %>%
  ...
  midpoints.line = transects %>% filter(grepl("LINE", st_astext))
    if (nrow(midpoints.line) != 0) {
      ...
      midpoints.line = do.call(rbind, lapply(lineframe, rgeos::gCentroid, byid = TRUE)) %>% sp::spTransform(sp::CRS(getOption("nwasc.proj"))) %>%
  ...
  # -------- summarize species data and convert to wide form ------------------------------------------------------------
    if (!occurences) {
      ...
      dts.final = dts.final %>% summarise(count = sum(count)) %>%
        tidyr::spread(spp_cd, count, fill = 0) %>% select(-NOT_AN_OBSERVATION) %>%
        ungroup %>% arrange(transect_id)
    }
    else if (occurences) {
      # number of species occurences
      dts.final = dts.final %>% select(-count) %>% summarise(noccur = n()) %>%
        tidyr::spread(spp_cd, noccur, fill = 0) %>% select(-NOT_AN_OBSERVATION) %>%
        ungroup %>% arrange(transect_id)
    }
